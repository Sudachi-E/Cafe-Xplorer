...
static DRESULT cache_readSectors(driveCache* cache, LBA_t sectorIdx, UINT sectorCount, BYTE* outputBuff) {
    for (uint32_t i=0; i<sectorCount; i++) {
        ...
        if (!alignedBuffer) return RES_ERROR;
        DRESULT rawRes = cache_doRawRead(cache, sectorIdx+i, sectorCount-i, alignedBuffer);
        if (rawRes != RES_OK) { free(alignedBuffer); return rawRes; }
        for (uint32_t f=i; f<i+readRange; f++) {
            ...
            if (outputBuff != nullptr)
                memcpy(outputBuff+(f*cache->sectorSize), newPtr->buffer, cache->sectorSize);
        }
        free(alignedBuffer);
        i += (readRange-1);
    }
...
DRESULT ffcache_initialize(BYTE pdrv, DWORD sectorByteSize, DWORD sectorCount) {
    ...
    if (!fatCaches[pdrv].sectorsBuffer) {
        OSReport("[ffcache] Failed to allocate %u bytes for cache\n", fatCaches[pdrv].sectorCount*fatCaches[pdrv].sectorSize);
        return RES_ERROR;
    }
    for (uint32_t i=0; i<sectorCount; i++) {
...
DRESULT ffcache_readSectors(BYTE pdrv, LBA_t sectorOffset, UINT sectorCount, BYTE *bufferOut) {
    ...
    return cache_readSectors(&fatCaches[pdrv], sectorOffset, sectorCount, bufferOut);
}
