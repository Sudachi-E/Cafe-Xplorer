...
public:
    ...
    bool update(int32_t width, int32_t height) {
        ...
        if (kpad.extensionType == WPAD_EXT_PRO_CONTROLLER) {
            ...
            data.buttons_h = MapWiiButtons(kpad.hold) | MapClassicButtons(kpad.classic.hold);
            data.buttons_d = MapWiiButtons(kpad.trigger) | MapClassicButtons(kpad.classic.trigger);
            data.buttons_r = MapWiiButtons(kpad.release) | MapClassicButtons(kpad.classic.release);
        ...
        } else if (kpad.extensionType == WPAD_EXT_NUNCHUK || kpad.extensionType == WPAD_EXT_MPLUS_NUNCHUK) {
            data.buttons_h = MapWiiButtons(kpad.hold) | MapNunchukButtons(kpad.nunchuk.hold)
                           | NunchukStickToInputButtons(kpad.nunchuk.stick);
            data.buttons_d = MapWiiButtons(kpad.trigger) | MapNunchukButtons(kpad.nunchuk.trigger);
            data.buttons_r = MapWiiButtons(kpad.release) | MapNunchukButtons(kpad.nunchuk.release);
            data.leftStickX  = kpad.nunchuk.stick.x;
            data.leftStickY  = kpad.nunchuk.stick.y;
            data.rightStickX = 0.0f;
            data.rightStickY = 0.0f;
        } else if (kpad.extensionType == WPAD_EXT_CORE || kpad.extensionType == WPAD_EXT_MPLUS) {
...
private:
    ...
    static uint32_t MapClassicButtons(uint32_t p) {
        uint32_t m = 0;
        if (p & WPAD_CLASSIC_BUTTON_A)      m |= Input::BUTTON_A;
        if (p & WPAD_CLASSIC_BUTTON_B)      m |= Input::BUTTON_B;
        if (p & WPAD_CLASSIC_BUTTON_X)      m |= Input::BUTTON_X;
        if (p & WPAD_CLASSIC_BUTTON_Y)      m |= Input::BUTTON_Y;
        if (p & WPAD_CLASSIC_BUTTON_LEFT)   m |= Input::BUTTON_LEFT;
        if (p & WPAD_CLASSIC_BUTTON_RIGHT)  m |= Input::BUTTON_RIGHT;
        if (p & WPAD_CLASSIC_BUTTON_UP)     m |= Input::BUTTON_UP;
        if (p & WPAD_CLASSIC_BUTTON_DOWN)   m |= Input::BUTTON_DOWN;
        if (p & WPAD_CLASSIC_BUTTON_PLUS)   m |= Input::BUTTON_PLUS;
        if (p & WPAD_CLASSIC_BUTTON_MINUS)  m |= Input::BUTTON_MINUS;
        if (p & WPAD_CLASSIC_BUTTON_HOME)   m |= Input::BUTTON_HOME;
        if (p & WPAD_CLASSIC_BUTTON_L)      m |= Input::BUTTON_L;
        if (p & WPAD_CLASSIC_BUTTON_R)      m |= Input::BUTTON_R;
        if (p & WPAD_CLASSIC_BUTTON_ZL)     m |= Input::BUTTON_ZL;
        if (p & WPAD_CLASSIC_BUTTON_ZR)     m |= Input::BUTTON_ZR;
        return m;
    }

    static uint32_t MapNunchukButtons(uint32_t p) {
        uint32_t m = 0;
        if (p & WPAD_NUNCHUK_BUTTON_C)      m |= Input::BUTTON_ZR;
        if (p & WPAD_NUNCHUK_BUTTON_Z)      m |= Input::BUTTON_ZL;
        return m;
    }

    static uint32_t NunchukStickToInputButtons(const KPADVec2D &stick) {
        uint32_t m = 0;
        if (stick.x > 0.25f)  m |= Input::BUTTON_RIGHT;
        if (stick.x < -0.25f) m |= Input::BUTTON_LEFT;
        if (stick.y > 0.25f)  m |= Input::BUTTON_UP;
        if (stick.y < -0.25f) m |= Input::BUTTON_DOWN;
        return m;
    }
};
